...
const AnswerConfig = ({ exam, onSave, onClose }) => {
  ...
  const setP1 = (n, c) => setLocalAns(prev => { 
    const p1 = {...prev.p1}; 
    p1[n] = c; 
    return { ...prev, p1: p1 }; 
  });

  const setP2 = (n, s, v) => setLocalAns(prev => { 
    const p2 = {...prev.p2}; 
    if (!p2[n]) p2[n] = {}; 
    p2[n][s] = v; 
    return { ...prev, p2: p2 }; 
  ...
  const setP3 = (n, v) => setLocalAns(prev => { 
    const p3 = {...prev.p3}; 
    p3[n] = v; 
    return { ...prev, p3: p3 }; 
  });
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in duration-200">
        ...
        <div className="p-8 border-t bg-slate-50 flex justify-end gap-4">
          ...
          <button onClick={() => onSave(localAns)} className="px-12 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all uppercase tracking-widest text-[11px]">LƯU ĐÁP ÁN</button>
        </div>
      </div>
    </div>
  );
